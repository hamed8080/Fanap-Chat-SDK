...
extension CMReplyInfo{
	...
	public class func convertReplyInfoToCM(replyInfo:ReplyInfo , messageId:Int? ,entity:CMReplyInfo? = nil) -> CMReplyInfo{
	...
	public class func insertOrUpdate(replyInfo:ReplyInfo , messageId:Int?,  resultEntity:((CMReplyInfo)->())? = nil){
        
		if let messageId = messageId, let findedEntity = CMReplyInfo.crud.find(keyWithFromat: "messageId == %i", value: messageId){
			let cmReplyInfo = convertReplyInfoToCM(replyInfo: replyInfo, messageId: messageId, entity: findedEntity)
            resultEntity?(cmReplyInfo)
        }else{
			CMReplyInfo.crud.insert { cmLinkedUserEntity in
				let cmLinkedUser = convertReplyInfoToCM(replyInfo: replyInfo, messageId: messageId, entity: cmLinkedUserEntity)
